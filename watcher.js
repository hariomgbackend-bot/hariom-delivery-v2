 ...
 *
 *  Setup:
 *   npm install chokidar firebase-admin express dotenv
 *
 *  Run manually:   node watcher.js
 *  Run as service: npm install -g node-windows  (see README below)
 * ════════════════════════════════════════════════════════════════
 */
...
function extractCustomerFromFilename(filename) {
  ...
  // Pattern: "Customer Name - INV/001"  OR  "Customer Name_INV001"
  ...
  return base; // fallback: whole filename
}
...
// Track in-progress uploads to avoid duplicate triggers (chokidar
// sometimes fires 'add' twice for the same file on Windows)
...
async function uploadInvoice(localFilePath) {
  ...
  try {
    // Wait briefly — some apps (Tally) keep the file handle open for
    // a moment after writing. 1.5 s is enough.
    ...
    // ── Check duplicate by invoiceNo in Firestore ────────────────
    ...
    // Make the file downloadable via signed URL (valid 7 days — enough
    // for any invoice to be imported or auto-deleted)
    ...
    console.log(`[watcher] ✅ Uploaded: ${filename} → ${storageName}`);
  } catch (err) {
...
function startWatcher() {
  // Create watch folder if it doesn't exist yet
  ...
  chokidar.watch(WATCH_FOLDER, {
    persistent:        true,
    ignoreInitial:     false,   // also upload PDFs already in the folder on startup
    awaitWriteFinish: {
      stabilityThreshold: 1000, // wait until file size stops changing
      pollInterval:       200,
    },
    depth: 0,                   // only top-level files, no subfolders
  }).on("add", (filePath) => {
    if (!filePath.toLowerCase().endsWith(".pdf")) return;
    console.log(`[watcher] New PDF detected: ${path.basename(filePath)}`);
    uploadInvoice(filePath);
  });
}

// ── Local API server (called by backend cron to delete local file) 
function startLocalApi() {
  ...
  /**
   * DELETE /local-file
   * Body: { localPath: "C:\\HariomDMS\\InvoiceWatch\\file.pdf" }
   * Called by the server.js cron after 8-hour auto-cleanup.
   * Only allows deleting files inside WATCH_FOLDER for safety.
   */
  app.delete("/local-file", (req, res) => {
    ...
    // Safety: only allow deleting files inside the watch folder
    const resolved = path.resolve(localPath);
    if (!resolved.startsWith(path.resolve(WATCH_FOLDER))) {
      console.warn(`[watcher-api] Blocked delete outside watch folder: ${resolved}`);
      return res.status(403).json({ error: "Path outside watch folder" });
    }

    if (!fs.existsSync(resolved)) {
      return res.json({ ok: true, note: "File already gone" });
    }

    try {
      fs.unlinkSync(resolved);
      console.log(`[watcher-api] 🗑 Deleted local file: ${resolved}`);
      res.json({ ok: true });
    } catch (err) {
      console.error(`[watcher-api] Delete failed: ${err.message}`);
  ...
  app.listen(LOCAL_API_PORT, "127.0.0.1", () => {
    console.log(`[watcher-api] 🟢 Local API running on port ${LOCAL_API_PORT}`);
  });
}
...
console.log(`
════════════════════════════════════════════
  Hariom DMS — Invoice Watcher  ✅ Running
...
/*
 * ════════════════════════════════════════════════════════════════
 *  INSTALL AS WINDOWS SERVICE (run once, then it auto-starts)
 *
 *  1. npm install -g node-windows
 *  2. Create install.js in this folder:
 *
 *     const { Service } = require("node-windows");
 *     const svc = new Service({
 *       name: "HariomInvoiceWatcher",
 *       description: "Hariom DMS – Cloud Invoice Watcher",
 *       script: require("path").join(__dirname, "watcher.js"),
 *       nodeOptions: ["--experimental-vm-modules"],
 *       env: [{ name: "WATCH_FOLDER", value: "C:\\HariomDMS\\InvoiceWatch" }]
 *     });
 *     svc.on("install", () => svc.start());
 *     svc.install();
 *
 *  3. node install.js
 *  4. Check Services app → "HariomInvoiceWatcher" should be Running
 *
 *  To uninstall:  svc.uninstall();
 * ════════════════════════════════════════════════════════════════
 */
